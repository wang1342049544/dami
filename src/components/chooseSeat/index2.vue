  ...
  <div class="choose-seat-container" :style="'height:' + containerHeight + 'px;'">
    ...
    <div id="seatMap" :class="'choose-seat-body '+ chartsSeatArea.join(' ') ">
      <div class="seat-charts-row" v-for="(item,x) in seatOrder" :style="'width:'+getItems(item).length*25+'px;'">
        <div v-for="(subItem,y) in getItems(item)" :class=" seatData[(x+1)+'_'+(y+1)].status==3?('seat-charts-spaces seat-gray-'+subItem):('seat-charts-cells seat-area-'+subItem)">
          <div v-if="subItem!='_' && seatData[(x+1)+'_'+(y+1)].status==1">
            <div v-if="subItem!='_' && seatData[(x+1)+'_'+(y+1)] " :id="(x+1)+'_'+(y+1)" :ref="(x+1)+'_'+(y+1)"
                 @click="clickCell" :class="'seat-charts-cell seat-charts-seat' ">
              ...
              <Tooltip placement="top">
                ...
                <div slot="content" style="z-index:10000;">
                  ...
                  <p>
                    状态：{{seatData[(x+1)+'_'+(y+1)].status==0?'没座位':seatData[(x+1)+'_'+(y+1)].status==1?'可售':seatData[(x+1)+'_'+(y+1)].status==2?'锁定待付款':'已售出'}}</p>
                </div>
              </Tooltip>
            </div>
          </div>
...
<!--          如果状态是2的话-->
          <div v-if="subItem!='_' && seatData[(x+1)+'_'+(y+1)].status==2">
            <div v-if="subItem!='_' && seatData[(x+1)+'_'+(y+1)] " :id="(x+1)+'_'+(y+1)" :ref="(x+1)+'_'+(y+1)" class="sofa-legend sofa-locked seat-charts-cell">
              <Tooltip placement="top">
                &nbsp; &nbsp;
                <div slot="content" style="z-index:10000;">
                  <p>座位：{{(x+1)}}排{{(y+1)}}号</p>
...
<!--          如果是3的话-->
          <div v-if="subItem!='_' && seatData[(x+1)+'_'+(y+1)].status==3">
            <div v-if="subItem!='_' && seatData[(x+1)+'_'+(y+1)] " :id="(x+1)+'_'+(y+1)" :ref="(x+1)+'_'+(y+1)"
